...
# # === Folder paths ===
# input_folder = r"T:\MediaStar XML\File Output"
# output_folder = r"T:\MediaStar XML\Differences"
# os.makedirs(output_folder, exist_ok=True)
...
# # === Step 1: Get the two most recent Excel files (excluding temp/lock files) ===
# xlsx_files = [
#     os.path.join(input_folder, f)
#     for f in os.listdir(input_folder)
...
# if len(xlsx_files) < 2:
#     raise Exception("Need at least two valid Excel files to compare.")
...
#     # 2. Modified eventIds (ignore case-only differences)
#     common_ids = set(new_df.index).intersection(old_df.index)
#     for eid in common_ids:
#         new_row = new_df.loc[eid]
#         old_row = old_df.loc[eid]
...
#     # 2. Deleted eventIds
#     deleted_ids = set(old_df.index) - set(new_df.index)
#     if deleted_ids:
#         deleted = old_df.loc[list(deleted_ids)].copy()
#         deleted['ChangeType'] = 'Deleted'
#         deleted['Sheet'] = sheet
#         diff_rows.append(deleted)

#     # 3. Modified eventIds (ignore case-only differences)
#     common_ids = set(new_df.index).intersection(old_df.index)
#     for eid in common_ids:
#         new_row = new_df.loc[eid]
#         old_row = old_df.loc[eid]
...
#     for row_num in range(2, ws.max_row + 1):
#         eid = str(ws.cell(row=row_num, column=eventId_idx).value)
#         change_type = ws.cell(row=row_num, column=col_index["ChangeType"]).value
...
##in other tabs the difference should be :
...
# # === Step 3: Compare sheet by sheet ===
# diff_by_sheet = {}             # sheet_name → DataFrame
# highlight_by_sheet = {}        # sheet_name → list of (eventId, col_name)
...
#             row_data = new_row.copy()
#             row_data['ChangeType'] = 'Modified'
#             rows.append(row_data.to_frame().T)
#             for col in modified_cols:
#                 highlights.append((eid, col))

#     if rows:
#         df = pd.concat(rows, ignore_index=True)
#         diff_by_sheet[sheet[:31]] = df
#         highlight_by_sheet[sheet[:31]] = highlights

# # === Step 4: Save all diffs to Excel ===
# if diff_by_sheet:
#     with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
#         for sheet, df in diff_by_sheet.items():
#             df.to_excel(writer, sheet_name=sheet, index=False)

#     # === Step 5: Apply highlights only to modified cells ===
#     wb = load_workbook(output_file)
#     yellow_fill = PatternFill(start_color="FFFF00", end_color="FFFF00", fill_type="solid")

#     for sheet_name, coords in highlight_by_sheet.items():
#         ws = wb[sheet_name]
#         headers = [cell.value for cell in ws[1]]
#         col_index = {col: idx + 1 for idx, col in enumerate(headers)}
#         eventId_col = col_index.get('eventId')
#         changeType_col = col_index.get('ChangeType')

#         if not eventId_col or not changeType_col:
#             continue

#         for row_num in range(2, ws.max_row + 1):
#             eid = str(ws.cell(row=row_num, column=eventId_col).value)
#             change_type = str(ws.cell(row=row_num, column=changeType_col).value)
#             if change_type != "Modified":
#                 continue
#             for col_name in headers:
#                 if (eid, col_name) in coords and col_name in col_index:
#                     ws.cell(row=row_num, column=col_index[col_name]).fill = yellow_fill

#     wb.save(output_file)
#     print(f"\n Sheet-by-sheet differences saved and highlighted at:\n{output_file}")
# else:
#     print("\n No differences found. No output file created.")


## with specific titles to be removed:
...
# Titles to ignore for added/deleted
ignore_titles = {
    "Local Programs",
...
for sheet in new_excel:
    ...
    if added_ids:
        added = new_df.loc[list(added_ids)].copy()
        added['ChangeType'] = 'Added'
        if 'Title' in added.columns:
            added = added[~added['Title'].astype(str).str.strip().isin(ignore_titles)]
        if not added.empty:
            rows.append(added)
    ...
    if deleted_ids:
        deleted = old_df.loc[list(deleted_ids)].copy()
        deleted['ChangeType'] = 'Deleted'
        if 'Title' in deleted.columns:
            deleted = deleted[~deleted['Title'].astype(str).str.strip().isin(ignore_titles)]
